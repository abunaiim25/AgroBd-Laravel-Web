...
class PaymentController extends Controller
{
    ...
    {
        $order_payment = OrderPayment::join('users',  'order_payments.user_id', 'users.id')->select('order_payments.*', 'users.email') //users table join on (orders.user_id) 
    ...
    {
        $order_payment = OrderPayment::join('users',  'order_payments.user_id', 'users.id')->select('order_payments.*', 'users.email') //users table join on (orders.user_id) = query join
            ->where('invoice_no', 'like', '%' . $request->search . '%')
            ->orWhere('email', 'like', '%' . $request->search . '%')
            ->orWhere('payment_type', 'like', '%' . $request->search . '%')
            ->orWhere('total', 'like', '%' . $request->search . '%')
